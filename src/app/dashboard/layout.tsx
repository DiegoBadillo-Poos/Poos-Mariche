...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  /**
   * VIGILANTE DE SESIÓN ÚNICA (PASIVO)
   * Solo corre dentro del Dashboard. Monitoriza si el lastSessionId cambia en la nube.
   */
  useEffect(() => {
    if (!firestore || !user || !auth) return;

    const unsubscribe = onSnapshot(doc(firestore, 'users', user.uid), async (snap) => {
      // 1. IGNORAR CAMBIOS LOCALES EN PROCESO (Escrituras optimistas)
      ...
      if (snap.exists()) {
        ...
        // 2. ADOPCIÓN AUTOMÁTICA
        // Si el usuario entra (ej. F5) y no tiene ID local, adopta el de la nube para no ser expulsado.
        if (!localSessionId && data.lastSessionId) {
          localStorage.setItem(SESSION_KEY, data.lastSessionId);
        } 
        // 3. EXPULSIÓN POR CONFLICTO
        // Solo si tenemos un ID local y es DIFERENTE al que está mandando en la nube.
      ...
      return (
          <div className="flex h-screen items-center justify-center bg-slate-50">
              <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-primary animate-pulse">Iniciando Sistema...</p>
              </div>
          </div>
      );
  }
